...
let createTransaction = async(username, pathParameter, body, stage) => {
    ...
    switch(body.mode) {
        case 'publish':
            ...
            transaction.from = '--';
            transaction.to = body.data.to;
            transaction.amount = parseInt(body.data.amount, 10);
            transaction.comment = body.data.comment ? body.data.comment : ' ';
            await controller.create(transaction);
            break;
        case 'send':
            if(body.data.from !== username && !await isAdmin(username)) {
                throw new Error("この取引を発行する権限がありません");
            }

            let amount = parseInt(body.data.amount, 10);
            if (await controller.getCarried(body.data.from) < amount) {
            ...
            transaction = {
                ...
                "to": body.data.to,
                "amount": amount,
                "comment": body.data.comment ? body.data.comment : ' ',
            };
            await controller.create(transaction);
            break;
        default:
            throw new Error("illigal mode");
    }
};
